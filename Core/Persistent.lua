...
--   Centralized access to V3_Settings SavedVariable
--=============================================================================
AutoLFM = AutoLFM or {}
...
function AutoLFM.Core.Persistent.MovePresetDown(presetName)
  if not presetName or not PresetsOrder or not characterID then return false end
  if not PresetsOrder[characterID] then return false end

  local order = PresetsOrder[characterID]
  for i = 1, table.getn(order) do
    if order[i] == presetName then
